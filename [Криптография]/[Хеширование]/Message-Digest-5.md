...
// k, s, i используются в преобразовании [abcd k s i] = ((a Fun(b,c,d) + X[k] + T[i]) <<< s)
var int s[64], K[64]
var int i

// s обозначает величины сдвигов для каждой операции: (тут все понятно)
...
// !!! капиталисты опять нас обманули !!! K[i] на самом деле это T[i] !!!
...
// Подготовка: добавляем бит "1" в конец сообщения. (0x80 можно так)
...
for each 512-bit chunk of padded message do
    ...
    break chunk into sixteen 32-bit words M[j], 0 ≤ j ≤ 15 // разбиваем "кусок" на 16 блоков по 32 бита
    ...
    for i from 0 to 63 do
        ...
        if 0 ≤ i ≤ 15 then
            ...
            F := C xor (B or (not D))
            g := (7×i) mod 16
        F := F + A + K[i] + M[g]  // M[g] — 32 битный блок !!! капиталисты опять нас провели. M[g] = X[k]
        A := D
        D := C
        C := B
